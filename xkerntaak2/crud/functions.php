...
function aanmeldingHandmatig()
{
    ...
    $sql = "INSERT INTO aanmelding (playerID, tournamentID) 
            VALUES (:player, :tournament)";
    $statement = $pdo->prepare($sql);

    $statement->bindValue(':player', $playerID);
    $statement->bindValue(':tournament', $tournamentID);

    $statement->execute();

    // echo 'Speler is aangemeld bij het toernooi';
    //         } catch (PDOException $e) {
    //             echo $e->getMessage();
    //         }
    //     } else {
    //         die('speler is al aangemeld bij dit toernooi');
    //     }
    // } catch (PDOException $e) {
    //     echo $e->getMessage();
    // }
...
// }include_once ('pages/aanmeldingen.php');
    // $playerID = trim($_POST["player"]);
    // $tournamentID = trim($_POST["tournament"]);
    // try {
    //     $stmt = $conn->prepare("SELECT * FROM aanmelding WHERE (playerID = '$playerID' AND tournamentID = '$tournamentID')");

    //     if($stmt->rowCount() == 0) {
    //         try {
    //             $stmt = $conn->prepare("INSERT INTO aanmelding (playerID, tournamentID) VALUES ('$playerID', '$tournamentID')");
    //             $stmt-> execute();
